...
export function LiveTVComments({ liveTVId, isAdmin = false }: LiveTVCommentsProps) {
  ...
  const loadComments = async () => {
    try {
      const response = await fetch(`/api/proxy/live-tv/${liveTVId}/comments`);
      if (response.ok) {
  ...
  const handleDelete = async (commentId: string) => {
    ...
    try {
      const response = await fetch(`/api/proxy/live-tv/comments/${commentId}`, {
        method: 'DELETE'
      });
  ...
  useEffect(() => {
    ...
    // Poll for new comments every 2 seconds for admin
    const interval = setInterval(loadComments, isAdmin ? 2000 : 3000);
  ...
  return (
    <div className={`bg-white rounded-lg shadow-lg ${isAdmin ? 'h-[500px]' : 'h-80'} flex flex-col`}>
      {/* Header */}
